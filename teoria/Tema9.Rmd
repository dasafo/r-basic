...
```{r}
n = length(cw)
k1 = ceiling(sqrt(n)) #tomamos la parte entera superior de la raiz   de n con ceiling()
k1
```
...
histAbs = function(x,L) {
  h = hist(x, breaks = L, right = FALSE, freq = FALSE,
           xaxt = "n", yaxt = "n", col = "lightgray", #al igualar x/yaxt="n" hacemos que no aparezcan los ejes( luego mas abajo con la funcion axis() los renombramos)
...
histAbsCum = function(x,L) {
  ...
  h$density = cumsum(h$density) #frecuencias relativa acumuladas, que representa la altura de las barras
