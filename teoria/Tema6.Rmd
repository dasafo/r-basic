...
```{r}
x = sample(1:5, size = 12, replace = TRUE) #muestra aleatoria de 12 elementos numeradas del 1 al 5
x

#Ahora sacamos un muestreo aleatorio de los valores si y no, y lo convertimos a factor para podeer trabajar con variables cualitativas
...
```{r}
table(x) 
#nos indica el numero/nivel y debajo el numero de veces que ha salido(si por azar un nivel no ha salido, no se muestra, esto se arregla haciendo lo que pone más dalente en 'Tablas de frecuencias unidimensionales')

table(Respuestas) #nos indica las veces que ha aparecido si/nivel y no/nivel
```
...
```{r}
names(table(x)) #devuelve el nombre de los niveles de x

names(table(Respuestas)) #devuelve el nombre de los niveles de respuestas
```
...
```{r}
barplot(table(Sexo), col=c("darkred","lightgreen"), main="Diagrama de barras de 
...
```{r}
barplot(prop.table(table(Respuestas)),col=c("red","green"), main="Diagrama de barras de frecuencias 
...
```{r}
par(mfrow=c(1,2))
barplot(table(Respuestas), col=c("darkgreen"))
barplot(table(Respuestas), col=c("darkred","darkblue"))
par(mfrow=c(1,1))
```
