...
```{r}
table(edad) #frecuencias globales absolutas
```
...
```{r}
round(prop.table(table(edad)),3) #frecuencias globales relativas
cumsum(table(edad)) #frecuencias globales acumuladas
```

## Ejemplo 1

```{r}
round(cumsum(prop.table(table(edad))),3) #frecuancias relativas acumuladas 
```
...
```{r}
round(cumsum(prop.table(table(dados))),2) #frecuencias relativas acumuladas
dados.df = data.frame(Puntuacion = 1:6,
                      Fr.abs = as.vector(table(dados)), #'as.vector' para poder colocarla como columna
...
#con el which nos devuelve la posicion, con name el nombre de esa posicion y con numerico el numero de ese nombre de esa posicion
